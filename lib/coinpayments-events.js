...
class CPEventHandler {
    ...
    pending(data) {
        if(data.amount2 === data.received_amount) {
            ...
            global.database.pool.query("SELECT alertboxApiKey FROM `users` WHERE id=?", [data.item_name], function (err, res) {
                if(err) {
                    console.error("error in sql");
                    return;
                }
                console.log("Sending alert to " + res[0].alertboxApiKey);
                SocketManager.instance.socketManager.to(res[0].alertboxApiKey).emit("payment", {from: data.buyer_name, amount: data.net});
            });
        }
    }
    ...
    success(data) {
        ...
        global.database.pool.query("UPDATE `transactions` SET order_status=? WHERE order_id=?", [2, data.txn_id], function (err, res) {
            if(err) {
                console.error(err);
            }
            ...
            global.database.pool.query("UPDATE `ledger` SET ?? = ? WHERE `user_id` = ?", updateTo, function (err, res) {
                if(err) {
                    console.error("ERROR!");
                    console.error(err);
                }
