    ...
    pending(data) {
        if(data.amount2 === data.received_amount) {
            ...
            global.database.pool.query("SELECT alertboxApiKey FROM `users` WHERE id=?", [data.item_name], function (err, res) {
                ...
                console.log("Sending alert to " + res.alertboxApiKey);
                SocketManager.instance.socketManager.to(res.alertboxApiKey).emit("payment", {from: data.buyer_name, amount: data.net});
            });
        }
    }
