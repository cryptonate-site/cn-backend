    ...
    pending(data) {
        if(data.amount2 === data.received_amount) {
            ...
            global.database.pool.query("SELECT alertboxApiKey FROM `users` WHERE id=?", [data.item_name], function (err, res) {
                ...
                SocketManager.instance.socketManager.to(res[0].alertboxApiKey).emit("payment", {from: data.buyer_name, amount: data.amount2, currency: data.currency2, message: data.custom});
            });
        }
    }
